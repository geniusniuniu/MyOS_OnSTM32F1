...
/*********************全局变量*/
	...
	void OLED_I2C_Init(void)/*引脚初始化*/
	{
		...
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_OD;		//引脚初始化为复用开漏输出
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_9;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(GPIOB, &GPIO_InitStructure);
		
		RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);//开启AFIO时钟
		AFIO_REMAP_ENABLE(AFIO_MAPR_I2C1_REMAP);
		...
		I2C_InitStructure.I2C_OwnAddress1 = 0x11;				//自身地址，从机模式下才有效
	...
	void OLED_WriteCommand(uint8_t Command)
	{
		...
		// 等待起始信号完成 	
		while (I2C_CheckEvent(OLED_I2C_PORT, I2C_EVENT_MASTER_MODE_SELECT) != SUCCESS);	
		
		// 发送设备地址（写入方向）  
		I2C_Send7bitAddress(OLED_I2C_PORT, OLED_I2C_ADDR, I2C_Direction_Transmitter);
		while (I2C_CheckEvent(OLED_I2C_PORT, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED) != SUCCESS);
		
		I2C_Send7bitAddress(OLED_I2C_PORT, OLED_I2C_COMMAND, I2C_Direction_Transmitter);
		while (I2C_CheckEvent(OLED_I2C_PORT, I2C_EVENT_MASTER_BYTE_TRANSMITTING) != SUCCESS);
		
		// 硬件I2C发送数据 
		I2C_SendData(OLED_I2C_PORT, Command);  
		// 等待数据发送完成  
		while (I2C_CheckEvent(OLED_I2C_PORT, I2C_EVENT_MASTER_BYTE_TRANSMITTED) != SUCCESS);
		
		I2C_GenerateSTOP(OLED_I2C_PORT, ENABLE);			//硬件I2C生成终止条件
	}
	...
	void OLED_WriteData(uint8_t* buffer, size_t buff_size)
	{
		...
		I2C_Send7bitAddress(OLED_I2C_PORT, OLED_I2C_DATA, I2C_Direction_Transmitter);
		while (I2C_CheckEvent(OLED_I2C_PORT, I2C_EVENT_MASTER_BYTE_TRANSMITTING) != SUCCESS);
		// 发送数据缓冲区  
...
void OLED_Init(void)
{
	...
	for(int i = 20000; i > 0; i--);	//等待一段时间OLED供电稳定
